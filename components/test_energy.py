...
def test_EnergyMeter_init__default_threshold():
    em = EnergyMeter()
    assert em.threshold == DEFAULT_THRESHOLD


def test_EnergyMeter_init__energy_is_random():
    em = EnergyMeter()
    assert em.energy >= 0 and em.energy <= DEFAULT_THRESHOLD


def test_EnergyMeter_init__set_threshold():
    THRESHOLD = 20
...
def test_EnergyMeter_init__0_threshold__raises_ValueError():
...
def test_EnergyMeter_burn_turn__success_returns_True():
    em = EnergyMeter(threshold=10)
    em.add_energy(10)
